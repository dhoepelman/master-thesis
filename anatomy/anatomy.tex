...
The by a large margin most widely used spreadsheet system is Microsoft Office Excel, which has a self-reported install base of 1.2 billion users \cite{microsoftByTheNumbers}.
Two less-popular but still common implementation are Apache OpenOffice Calc and LibreOffice Calc, which evolved from the same product and thus have largely identical semantics, of which LibreOffice a self-reported install base of 75 million users.
...
There are many other spreadsheet implementations, but none come close to Microsoft Excels market share, and most are smaller than LibreOffice and Google Spreadsheets.
When referencing "all" spreadsheet programs, this is meant as the three implementations previously mentioned.
...
Formulas consist of an expression which can contain constant values, \key{function calls} and \key{operators} and, most importantly, \key{references} to other cells.
