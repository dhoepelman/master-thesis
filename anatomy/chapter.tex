...
Currently two major formula language exist in spreadsheets. The first is the Office Open XML spreadsheet language which is a standarization of the Excel language, which is currently used by Microsoft Excel.
...
Function calls are performed by starting with the function name, followed by the arguments in parentheses, separated by a comma.
...
Spreadsheet program contain three fairly unique binary operators: the range operator \f{:}, the union operator \f{,} in Excel, \f{~} in OpenFormula and the intersection operator \f{\char32} in Excel, \f{!} in OpenFormula.
...
\todo{Screenshot van copy-pasten van cell}
...
\todo{Plaatje van R1C1 en A1 style formule naast elkaar}
