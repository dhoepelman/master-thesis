...
This is not a limitation inherent to the refactorings, the previous state could easily be remembered and some refactorings even have a well-defined inverse refactoring.
...
If the user could select the formula to be extracted from within the Excel formula editor the user interface could be streamlined, but this is not possible because of Excel limitations.
...
The \rf{Introduce Aggregate} automated refactoring currently only supports a very basic scenario.
This could be improved by properly determining functional dependencies using an established algorithm.
The conditional aggregate functions function \f{SUMIF}, \f{COUNTIF} and \f{AVERAGEIF} are higher-order functions because they accept a predicate (a function which takes a single argument and produces a boolean), although Excel does not support first-class functions and thus the predicate must be supplied as a string.
This means more complex situations can be supported, for example \f{SUMIF(A:A, ">10")} sums every value from column \f{A} that is larger than \f{10}.
The conditional aggregate functions also have variants that support more than one predicate: \f{SUMIFS}, \f{AVERAGEIFS} and \f{COUNTIFS}.
For example \f{=SUMIFS(A:A,B:B,2015,C:C,"John")} sums the values from column \f{A} of the rows where column \f{B} contains \f{2015} and column \f{C} contains \f{"John"}.
This can be used to support multivalued dependencies, functional dependencies where two or more columns determine if a value must be summed.